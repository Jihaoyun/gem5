...
def StartRegFaultSim(statFolder, benchmark, fault):
	cmd = ["./build/ARM/gem5.opt", 
		...
		"configs/lapo/multi_reg_fault_injector.py",
		"-fe",
		"-b", benchmark,
		"-l", fault.label,
	...
	#cmd.insert(1, "--debug-start=" + str(320000000))
...
if __name__ == '__main__':
	for benchmark in args.benchmarks:
		...
		statFolder = benchmark.split("/")[-1] + "-multiIntermRegFault-ARM-stat"
		...
		try:
			...
			if args.faultInput is not None:
				for inputFile in args.faultInput.split(" "):
					fp = MultiIntermRegFaultParser(inputFile)
					if args.multiThread:
						tpool = []
					while fp.hasNext():
						fe = fp.next()
