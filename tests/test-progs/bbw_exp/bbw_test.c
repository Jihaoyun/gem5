...
__attribute__((always_inline)) inline double findSign(double inputNum){
	double returnVal;
	if (inputNum > 0)
		returnVal = 1;
	else if (inputNum < 0)
...
__attribute__((always_inline)) inline double findSum(double inputArr[]){
	int i;
	double returnVal = 0;
	for (i=0;i<4;i++)
...
__attribute__((always_inline)) inline double MARSModelEval(double inputVal){
	double f1;
	if (inputVal <= 39)
		f1 = 0;
