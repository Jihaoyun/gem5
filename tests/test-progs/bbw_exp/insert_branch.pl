...
=head2 ## Branch:
  ...
  -bi   <int>   (default = 10)    The interval of branch instruction and branch-back point
...
my $branch_int  = $args{"branch_enable"} ? $args{"branch_int"}  : 0;
...
my $branch_is_inserting = undef;       # If a branch is ready to be inserted but need to wait for the interval cycle
my $branch_int_cnt      = $branch_int; # Judge if need to wait for the interval

srand();
foreach my $line (@code_lines) {

    # Find the end of code (need to mannually insert DumbFunc() at first)
    if ($line =~ "^.*DumbFunc.*:" and not $line =~ "GLOBAL") {
    ...
    # Inserting the modified code line (if $branch_is_inserting is defined --- means still has one branch waited to be inserted)
    if (($is_main_func and not $branch_insertion_finish) or $branch_is_inserting) {
        if (rand(1) < $branch_prob or $branch_is_inserting) {

            # Raise the flag if we are inserting a branch
            if (not $branch_is_inserting) {
                $branch_is_inserting  = 1;
            ...
            if ($branch_int_cnt == 0) {
                ...
                $branch_int_cnt      = $branch_int;
                $branch_is_inserting = undef; 
            }
            else {
                $branch_int_cnt -= 1;                
            }
    	...
    	if ($line =~ "bl.*DumbFunc") {
            ...
            # No branch is pending to be inserted
            $branch_is_inserting = undef;
    	}
    }
